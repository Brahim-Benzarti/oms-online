    ...
    if($_FILES['profile_picture']['name']){
        if($_FILES['profile_picture']['error']){
            ...
            if($_FILES['profile_picture']['size'] > 5120000){
                ...
                mysqli_query($conn,$req)or die(mysqli_error($conn));
                move_uploaded_file($_FILES['profile_picture']['tmp_name'], $_SERVER['DOCUMENT_ROOT']."/public/images/users/".$file_name)or die($_SERVER['DOCUMENT_ROOT']."/public/images/users/".$file_name);
            }
        }
    }
}else{
